...
{
    if (argc < 5) {
        fprintf(stderr, "usage: %s <file_a> <file_b> <out_a> <out_b> [threads] [max_trials_per_side]\n", argv[0]);
        return 1;
    }
    ...
    uint64_t max_trials = (argc > 6) ? strtoull(argv[6], NULL, 10) : ((uint64_t)1 << 26);

    pdf_buffer_t a = pdf_load(path_a);
    pdf_buffer_t b = pdf_load(path_b);

    attack_result_t r = birthday_attack(a.data, a.size, b.data, b.size, STUDENT_ID, threads, max_trials);
